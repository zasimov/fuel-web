...
$(BUILD_DIR)/mirror/rhel/yum.done: \
		...
		$(call find-files,$(BUILD_DIR)/repos/fuel/deployment/puppet/rpmcache/files/required-rpms.txt \
		$(BUILD_DIR)/mirror/rhel/yum-config.done
	yum -c $(BUILD_DIR)/mirror/rhel/etc/yum.conf clean all
	rm -rf /var/tmp/yum-$$USER-*/
	grep -v "^\\s*\#" $(BUILD_DIR)/repos/fuel/deployment/puppet/rpmcache/files/required-rpms.txt | /bin/sed 's/-[0-9][0-9\.a-zA-Z_-]\+//g' | xargs \
