...
$(BUILD_DIR)/mirror/eggs/build.done: $(BUILD_DIR)/repos/fuel.done
$(BUILD_DIR)/mirror/eggs/build.done: $(call find-files,$(BUILD_DIR)/repos/fuel/deployment/puppet/nailgun/files/venv-ostf.txt)
...
$(BUILD_DIR)/mirror/eggs/build.done: \
	...
	sudo chroot $(SANDBOX) pip install \
		...
		$(REQUIRED_EGGS)
	grep -v "^\\s*\#" $(BUILD_DIR)/repos/fuel/deployment/puppet/nailgun/files/venv-ostf.txt | xargs \
	sudo chroot $(SANDBOX) pip install \
		--exists-action=i \
		--index-url $(MIRROR_EGGS) \
		--find-links $(MIRROR_EGGS) \
