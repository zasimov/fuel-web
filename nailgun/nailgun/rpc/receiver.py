...
class NailgunReceiver(object):
    ...
    def remove_nodes_resp(cls, **kwargs):
        ...
        for node in nodes:
            node_db = db.session.query(Node).get(node['uid'])
            if not node_db:
                logger.error(
    ...
    def remove_cluster_resp(cls, **kwargs):
        ...
        if task.status in ('ready',):
            ...
            ips = db.session.query(IPAddr).filter(
                IPAddr.network.in_([n.id for n in nws])
            )
            map(db.session.delete, ips)
            db.session.commit()
    ...
    def deploy_resp(cls, **kwargs):
        ...
        if nodes and not progress:
            nodes_progress = []
            nodes_db = db.session.query(Node).filter_by(
    ...
    def _generate_error_message(cls, task, error_types, names_only=False):
        nodes_info = []
        error_nodes = db.session.query(Node).filter_by(
            cluster_id=task.cluster_id
        ).filter(
            or_(
                Node.status == 'error',
    ...
    def _success_action(cls, task, status, progress):
        ...
        if task.cluster.mode in ('singlenode', 'multinode'):
            ...
            controller = db.session.query(Node).filter_by(
                cluster_id=task.cluster_id,
                role='controller'
            ).first()
            if controller:
    ...
    def verify_networks_resp(cls, **kwargs):
        ...
        if nodes is None:
            ...
            if forgotten_uids:
                absent_nodes = db.session.query(Node).filter(
